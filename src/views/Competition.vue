...
export default {
  ...
  computed: {
    ...mapState(["allCompetitions"]),
    categories() {
      return this.allCompetitions
        ? this.allCompetitions.find((data) => data.id == this.$route.params.id)
        : null;
    },
  },
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-md-8 mb-5 heading-section text-center">
            <span class="subheading" v-if="categories">{{
              categories.name
            }}</span>
          </div>
        </div>
        <div class="row d-flex">
          <div
            @click="$router.push(`/competitions/details/${competition.id}`)"
            v-if="categories"
